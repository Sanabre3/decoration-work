  ...
  const handleUploadArquivo = (event) => {
    ...
    reader.onload = (e) => {
      ...
      linhas.forEach((linha, index) => {
        if (index === 0 && linha.toLowerCase().includes('responsavel')) return; // Skip header
        
        const valores = linha.split(/[,;\t]/).map(v => v.trim());
        if (valores.length >= 2) {
          novosConvidados.push({
            responsavel: valores[0] || '',
            nome: valores[1] || '',
            tipo: valores[2] || 'Adulto',
            contato: valores[3] || '',
            data: valores[4] || '-',
            idade: parseInt(valores[5]) || 0
          });
        }
      });

      if (novosConvidados.length > 0) {
        atualizarDadosEvento({ ...dadosEvento, convidados: [...convidados, ...novosConvidados] });
        alert(`${novosConvidados.length} convidados importados com sucesso!`);
      }
    };
    ...
    <div className="page">
      <header className="page-header">
        ...
        <div className="header-buttons">
          <label className="btn-secondary" style={{cursor: 'pointer', marginRight: '10px'}}>
            📤 Importar Lista
            <input 
              type="file" 
