...
const Dashboard = () => {
  ...
  const { eventoAtual, eventos } = useEventos();
  ...
  const handleNavigate = (pagina) => {
    ...
    if (pagina === 'eventos') {
      ...
      navigate('/');
    } else if (pagina === 'original') {
      window.alert('Redirecionamento para versão original HTML (index.html)');
      // Em produção: window.location.href = '/index.html';
    } else {
